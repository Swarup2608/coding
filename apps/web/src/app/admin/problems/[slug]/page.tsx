...
function toFormValues(problem: Problem): ProblemFormValues {
  return {
    ...
    status: problem.status as ProblemFormValues["status"],
...
export default function EditProblemPage() {
  ...
  async function handleUpdate(values: ProblemFormValues) {
    await apiRequest(`/problems/${slug}`, {
      method: "PATCH",
      body: JSON.stringify({
        title: values.title,
  ...
  return (
    <main className="p-10">
      ...
      <TestCaseManager problemId={problem._id} />
    </main>
  );
}

function TestCaseManager({ problemId }: { problemId: string }) {
  ...
  async function loadTestCases() {
    const response = await apiRequest<{ success: boolean; data: TestCase[] }>(`/test-cases/problem/${problemId}`);
  ...
  async function handleAdd(event: React.FormEvent) {
    ...
    try {
      await apiRequest(`/test-cases/problem/${problemId}`, {
        method: "POST",
        body: JSON.stringify({ input, expectedOutput, isSample }),
      });
